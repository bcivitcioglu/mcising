...
| Update algorithms | Metropolis, Wolff, Swendsen-Wang, Wang-Landau with multicanonical production and reweighting | Metropolis, Gibbs, Wolff, Swendsen-Wang (interleaved with single-spin sweeps), replica overlap-cluster moves | local and cluster updates | Metropolis, Wolff (from the source tree) | Metropolis |
...
- **Flat-histogram sampling with the evidence attached.** Wang-Landau estimation of the density of states on an exact energy grid, a frozen-weight multicanonical production run, canonical reweighting to any temperature with jackknife errors, and the round-trip, flatness and effective-sample-size diagnostics that say whether the barrier was actually crossed — see the [Wang-Landau tutorial](../tutorial/wang-landau.md).
